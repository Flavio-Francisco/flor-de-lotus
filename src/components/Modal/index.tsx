...
import { calcularIdade, formatarTelefone } from "../../utils/Function";
...
export default function ModalObs({
    ...
    const [isChecked, setChecked] = useState(child?.ChildInformation.allergy);
    const [isCheckedhealth, setCheckedhealth] = useState(child?.ChildInformation.HealthInsurance);
    const [daily, setDaily] = useState(child?.ImportantInformation.Daily);
    const [overnight, setOvernight] = useState(child?.ImportantInformation.overnight);
    const [travel, setTravel] = useState(child?.ImportantInformation.travel);
    const [stroll, setStroll] = useState(child?.ImportantInformation.stroll);
    ...
    return (
        <>
            ...
            <ConteinerButtom>
                    <ButtomModal onPress={hideModalHandler} style={{ backgroundColor: colorChold(child?.ChildGender) }}>
                        <TextButtom>Fechar</TextButtom>
                    </ButtomModal>
                    <ButtomModalDelete onPress={ModalHandlerDelete} style={{ backgroundColor: colorChold(child?.ChildGender) }}>
